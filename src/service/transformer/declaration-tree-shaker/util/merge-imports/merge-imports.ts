...
export function mergeImports(statements: Statement[]): Statement[] {
  ...
  const namedImportsFromModulesMap: Map<
    string,
    Array<Array<{ propertyName: string; alias: string }>>
  > = new Map()
  ...
  for (const importDeclaration of imports) {
    ...
    const addAliasForNamedImport = (propertyName: string, alias: string) => {
      let collectionWithProperty = namedImportsFromModules.find(records =>
        records.some(
          record =>
            propertyName === record.propertyName && alias === record.alias,
        ),
      )
    ...
    let defaultImportsFromModules = defaultImportsFromModulesMap.get(
      specifierText,
    )
    ...
    let namespaceImportsFromModules = namespaceImportsFromModulesMap.get(
      specifierText,
    )
    if (namespaceImportsFromModules == null) {
      namespaceImportsFromModules = new Set()
      namespaceImportsFromModulesMap.set(
        specifierText,
        namespaceImportsFromModules,
      )
    }
    ...
    if (importDeclaration.importClause == null) {
      ...
      if (importDeclaration.importClause.namedBindings != null) {
        if (isNamespaceImport(importDeclaration.importClause.namedBindings)) {
          namespaceImportsFromModules.add(
            importDeclaration.importClause.namedBindings.name.text,
          )
        } else {
    ...
    for (const name of names) {
      importDeclarations.add(
        createImportDeclaration(
          undefined,
          undefined,
          createImportClause(createIdentifier(name), undefined),
          createStringLiteral(module),
        ),
      )
    }
    ...
    for (const name of names) {
      importDeclarations.add(
        createImportDeclaration(
          ...
          createImportClause(
            undefined,
            createNamespaceImport(createIdentifier(name)),
          ),
          createStringLiteral(module),
        ),
      )
            ...
            createNamedImports(
              collection.map(record =>
                record.propertyName !== record.alias
                  ? createImportSpecifier(
                      createIdentifier(record.propertyName),
                      createIdentifier(record.alias),
                    )
                  : createImportSpecifier(
                      undefined,
                      createIdentifier(record.alias),
