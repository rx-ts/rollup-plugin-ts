...
export function mergeExports(statements: Statement[]): Statement[] {
  ...
  const moduleSpecifierToExportedBindingsMap: Map<
    string,
    Set<string>
  > = new Map()
  ...
  for (const exportDeclaration of exports) {
    ...
    const specifierText =
      exportDeclaration.moduleSpecifier == null
        ? EMPTY_MODULE_SPECIFIER_TOKEN
        : (exportDeclaration.moduleSpecifier as { text?: string }).text

    let exportedBindings = moduleSpecifierToExportedBindingsMap.get(
      specifierText,
    )
      ...
      if (aliasedExportSpecifiers.size > 0) {
        exportDeclarations.add(
          createExportDeclaration(
            undefined,
            undefined,
            createNamedExports([...aliasedExportSpecifiers]),
  ...
  for (const [
    specifier,
    exportedBindings,
  ] of moduleSpecifierToExportedBindingsMap) {
    ...
    exportDeclarations.add(
      createExportDeclaration(
        ...
        undefined,
        createNamedExports(
          [...exportedBindings].map(exportedBinding =>
            createExportSpecifier(undefined, exportedBinding),
          ),
        ),
        specifier === EMPTY_MODULE_SPECIFIER_TOKEN
          ? undefined
          : createStringLiteral(ensureHasLeadingDotAndPosix(specifier)),
      ),
    )
  }
