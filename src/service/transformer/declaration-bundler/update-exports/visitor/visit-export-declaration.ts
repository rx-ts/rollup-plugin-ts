...
export function visitExportDeclaration({
...
}: UpdateExportsVisitorOptions<ExportDeclaration>):
  | ExportDeclaration
  | undefined {
  ...
  const isExportStar =
    ...
    symbol != null && (symbol.flags & SymbolFlags.ExportStar) !== 0
    ...
    if (
      ...
      if (node.exportClause != null) {
        for (const element of node.exportClause.elements) {
          const ref =
            element.propertyName != null ? element.propertyName : element.name
          ...
          if (declaration != null) {
            element.name.text === 'default'
              ? identifiersForDefaultExportsForModules.set(
                  ...
                  [ref.text, declaration],
                )
              : parsedExportedSymbols.set(
                  ref.text,
                  (declaration as unknown) as DeclarationStatement,
                )
          }
        }
      }
  ...
  if (isSameChunk) {
    if (!isEntry) {
      ...
      if (isExportStar) {
        const absoluteModuleSpecifierText = join(
          dirname(normalize(sourceFile.fileName)),
          specifier.text,
        )
        const missingExportSpecifiers = [
          ...getParsedExportedSymbolsForModule(
            absoluteModuleSpecifierText,
          ).keys(),
        ].filter(
          parsedExportedSymbol =>
            !getExportedSpecifiersFromModule(absoluteModuleSpecifierText).has(
              parsedExportedSymbol,
            ),
        )
    ...
    if (isExportStar) {
      ...
      if (missingExportSpecifiers.length < 1) {
        return undefined
      }
      missingExportSpecifiers.forEach(exportedSymbol =>
        getExportedSpecifiersFromModule(absoluteModuleSpecifierText).add(
          exportedSymbol,
        ),
      )
      return createExportDeclaration(
        ...
        createNamedExports(
          [...missingExportSpecifiers].map(exportSymbol =>
            createExportSpecifier(undefined, exportSymbol),
          ),
        ),
      )
    } else {
      // Walk through all of the named exports.
      const exportSpecifiersWithReplacements: Map<
        ExportSpecifier,
        ExportSpecifier
      > = new Map()
      if (node.exportClause != null) {
              ...
              if (identifiersForDefaultExportsForModules.has(path)) {
                ...
                if (element.propertyName != null) {
                  exportSpecifiersWithReplacements.set(
                    element,
                    createExportSpecifier(
                      identifiersForDefaultExportsForModules.get(path)[0],
                      element.name,
                    ),
                  )
                } else {
      ...
      const updatedSpecifiers = [
        ...(node.exportClause == null
          ...
          : node.exportClause.elements.filter(
              element => !exportSpecifiersWithReplacements.has(element),
            )),
        ...exportSpecifiersWithReplacements.values(),
      ]

      updatedSpecifiers.forEach(updatedSpecifier =>
        exportedSpecifiersFromModule.add(
          updatedSpecifier.propertyName != null
            ? updatedSpecifier.propertyName.text
            : updatedSpecifier.name.text,
        ),
      )
    ...
    return updateExportDeclaration(
      ...
      node.modifiers,
      node.exportClause,
      createStringLiteral(normalizedModuleSpecifier),
    )
  }
