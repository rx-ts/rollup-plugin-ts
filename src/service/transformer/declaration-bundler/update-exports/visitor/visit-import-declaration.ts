...
function createTypeAliasOrVariableStatementForIdentifier(
  ...
  switch (identifierNode.kind) {
        ...
        [
          createHeritageClause(SyntaxKind.ExtendsKeyword, [
            createExpressionWithTypeArguments(
              classDeclaration.typeParameters == null
                ? undefined
                : classDeclaration.typeParameters.map(param =>
                    createTypeReferenceNode(param.name, undefined),
                  ),
        ...
        [
          createHeritageClause(SyntaxKind.ExtendsKeyword, [
            createExpressionWithTypeArguments(
              classExpression.typeParameters == null
                ? undefined
                : classExpression.typeParameters.map(param =>
                    createTypeReferenceNode(param.name, undefined),
                  ),
              createIdentifier(identifier),
            ),
    ...
    case SyntaxKind.InterfaceDeclaration: {
      const declaration = identifierNode as
        | TypeAliasDeclaration
        | InterfaceDeclaration
...
export function visitImportDeclaration({
  ...
  if (isSameChunk) {
    if (node.importClause != null) {
      ...
      if (
        node.importClause.name != null &&
        identifiersForDefaultExportsForModulesPath != null
      ) {
        const [
          identifier,
          identifierNode,
        ] = identifiersForDefaultExportsForModules.get(
          identifiersForDefaultExportsForModulesPath,
        )
        ...
        if (identifier !== node.importClause.name.text) {
          newNodes.push(
            createTypeAliasOrVariableStatementForIdentifier(
              identifier,
              identifierNode,
      ...
      if (
        ...
        parsedExportedSymbolsPath != null
      ) {
        const specifiers = getParsedExportedSymbolsForModule(
          parsedExportedSymbolsPath,
        )
        ...
        if (isNamedImports(node.importClause.namedBindings)) {
          for (const element of node.importClause.namedBindings.elements) {
            if (
              ...
              specifiers.has(element.propertyName.text)
            ) {
            ...
            createModuleDeclaration(
              ...
              createModuleBlock(
                [...specifiers.values()].map(specifierNode => {
                  ...
                  clone.modifiers = createNodeArray(
                    removeExportModifier(clone.modifiers),
                  )
                  return clone
                }),
              ),
