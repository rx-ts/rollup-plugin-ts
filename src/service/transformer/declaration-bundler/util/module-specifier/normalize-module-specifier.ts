...
export function normalizeModuleSpecifier({
  ...
  const normalizedAbsoluteModuleSpecifierText = getChunkFilename(
  ...
  const normalizedModuleSpecifier = stripExtension(
    ensureHasLeadingDotAndPosix(
      relative(
        dirname(absoluteOutFileName),
        normalizedAbsoluteModuleSpecifierText,
      ),
    ),
  )
  ...
  return {
    ...
    normalizedAbsoluteModuleSpecifier: ensurePosix(
      normalizedAbsoluteModuleSpecifierText,
    ),
    hasChanged:
      normalize(ensurePosix(normalizedModuleSpecifier)) !==
      normalize(ensurePosix(specifier)),
    isSameChunk:
      normalize(ensurePosix(normalizedModuleSpecifier)) ===
      normalize(ensurePosix(stripExtension(relativeOutFileName))),
  }
}
