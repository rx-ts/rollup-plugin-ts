...
export function getAliasedDeclaration(
  node: Expression | Symbol | undefined,
  ...
  try {
    symbol =
      node == null
        ? undefined
        : 'valueDeclaration' in node || 'declarations' in node
        ? node
  ...
  try {
    ...
    if (
      ...
      (aliasedDeclaration.valueDeclaration != null ||
        (aliasedDeclaration.declarations != null &&
          aliasedDeclaration.declarations.length > 0))
    ) {
      valueDeclaration =
        aliasedDeclaration.valueDeclaration != null
