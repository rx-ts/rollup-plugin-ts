import {
  ...
  findRelativeConfig,
  // @ts-ignore
} from '@babel/core/lib/config/files/configuration'
// @ts-ignore
 ...
 */
export function isBabelInputOptions(
  babelConfig?: FindBabelConfigOptions['babelConfig'],
): babelConfig is IBabelConfig {
...
export function findBabelConfig({
  babelConfig,
  cwd,
}: FindBabelConfigOptions): FindBabelConfigResult | undefined {
  ...
  const relativeConfigResult = findRelativeConfig(
    pkgData,
    process.env.NODE_ENV,
  ) as { config: { filepath: string } | null } | null
