...
function getForcedModuleKindOption({
  rollupOutputOptions,
}: IGetForcedCompilerOptionsOptions): { module: ModuleKind } {
...
function getForcedScriptTargetOption({
  pluginOptions,
  browserslist,
}: IGetForcedCompilerOptionsOptions): { target?: ScriptTarget } {
 ...
 */
export function getForcedCompilerOptions(
  options: IGetForcedCompilerOptionsOptions,
