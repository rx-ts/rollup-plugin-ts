...
export function flattenDeclarationsFromRollupChunk({
...
}: IEmitDeclarationsOptions): IFlattenDeclarationsFromRollupChunkResult {
  const absoluteChunkFileName = join(outDir, chunk.fileName)
  const declarationFilename = setExtension(
    chunk.fileName,
    DECLARATION_EXTENSION,
  )
  const absoluteDeclarationFilename = join(
    declarationOutDir,
    declarationFilename,
  )
  const declarationMapFilename = setExtension(
    chunk.fileName,
    DECLARATION_MAP_EXTENSION,
  )
  const declarationMapDirname = join(
    declarationOutDir,
    dirname(declarationMapFilename),
  )
  const absoluteDeclarationMapFilename = join(
    declarationOutDir,
  ...
  const entrySourceFiles = entryFileNames.map(entryFileName =>
    program.getSourceFile(entryFileName),
  )
  const entrySourceFileSymbols = entrySourceFiles.map(entrySourceFile =>
    typeChecker.getSymbolAtLocation(entrySourceFile),
  )

  const usedExports: Set<string> = new Set()

  for (const entrySourceFileSymbol of entrySourceFileSymbols) {
    if (entrySourceFileSymbol == null) {
      continue
    }
    const exports = typeChecker
      .getExportsOfModule(entrySourceFileSymbol)
      ...
      if (replacedFile.endsWith(DECLARATION_MAP_EXTENSION)) {
        ...
        parsedData.sources = parsedData.sources
          .map(source => {
            ...
            // to cwd
            const sourceFromCwd = posixSource.startsWith('../')
              ? posixSource.slice('../'.length)
              : posixSource
  ...
  const result = transform(
    ...
    declarationTreeShaker({ relativeOutFileName: declarationFilename })
      .afterDeclarations as Array<TransformerFactory<SourceFile>>,
    languageServiceHost.getCompilationSettings(),
  )

  // Print the Source code and update the code with it
  code = createPrinter({
    newLine: languageServiceHost.getCompilationSettings().newLine,
