...
export function emitDiagnosticsThroughRollup({
  ...
  ;[
    ...
    const position =
      diagnostic.start == null || diagnostic.file == null
        ? undefined
        : diagnostic.file.getLineAndCharacterOfPosition(diagnostic.start)

    // Color-format the diagnostics
    const colorFormatted = formatDiagnosticsWithColorAndContext(
      [diagnostic],
      languageServiceHost,
    )

    // Provide a normalized error code
    const code = `${diagnostic.scope == null ? 'TS' : diagnostic.scope}${
      diagnostic.code
    }`
    ...
    let frame = colorFormatted.slice(
      colorFormatted.indexOf(message) + message.length,
    )
